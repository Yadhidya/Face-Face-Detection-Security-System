...
namespace Face_Detection_Security_System
{
    public partial class Database : Form
    {
            ...
            for (int i = 0; i < list.Length; i++)
            {
               ...
               //so that it does not gives IOException of image resource being used by another process
                using (FileStream stream = new FileStream(path[i], FileMode.Open, FileAccess.Read))
                {
                    ...
                    list[i].Title = lname;
                    string[] u_ids = uid[i].Split('*');
                    list[i].Message = "USER ID\t: " + u_ids[0] + "\nPhone Number\t:" + u_ids[2] + "\nAddress\t:" + u_ids[3] + "\nIn Time\t:" + u_ids[4] + "\nOut Time\t:" + u_ids[5];
        ...
        private void viewBtn_Click(object sender, EventArgs e)
        {
            ...
            string filepath = Directory.GetCurrentDirectory() + @"\PersonDetails\UID_Details.txt";
            string[] uids = File.ReadAllLines(filepath);
            for (int i = 0; i < uids.Length; i++)
            {
                uid.Add(uids[i]);
                string[] u_ids = uids[i].Split('*');
                string path = Directory.GetCurrentDirectory() + @"\TrainedImages\"+u_ids[0];
